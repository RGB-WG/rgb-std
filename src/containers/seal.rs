...
use rgb::{ExposedSeal, GenesisSeal, GraphSeal, SecretSeal, Xchain};
...
impl From<Xchain<GraphSeal>> for TerminalSeal {
    fn from(seal: Xchain<GraphSeal>) -> Self {
        match seal {
            Xchain::Bitcoin(
                seal @ GraphSeal {
    ...
    fn conceal(&self) -> Self::Concealed {
        match *self {
            ...
            TerminalSeal::BitcoinWitnessVout(seal) => {
                Xchain::Bitcoin(GraphSeal::from(seal)).conceal()
            }
            TerminalSeal::LiquidWitnessVout(seal) => {
                Xchain::Liquid(GraphSeal::from(seal)).conceal()
            }
        }
    }
}
...
impl From<Outpoint> for BuilderSeal<GenesisSeal> {
    fn from(outpoint: Outpoint) -> Self { BuilderSeal::Revealed(Xchain::Bitcoin(outpoint.into())) }
}
